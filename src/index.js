...
const adminOnly = (req, res, next) => {
  if (
    ...
    if (isAjaxCall(req)) {
      return res.status(405).json({ ok: 0, err: "Not allowed.", details: null })
    }
...
app.post(
  ...
  (req, res, next) => {
    ...
    function enqueueCommentNotification(_commentData) {
      return notificationsQueue.jobs.commentNotification(_commentData, req.user)
    }
...
app.post(
  ...
  (req, res, next) => {
    ...
    db.table("comment_boosts")
      ...
      .then((notificationsResult) => {
        return db("comment_boosts").select("id").where({ comment_id: cid })
      })
      .then((allBoostsResult) => {
        res.json({
          ok: 1,
...
function handleTextType(pid, req, res, next) {
  ...
  console.log("saving quick update with images...")
  db("products")
    .select()
    .where({ id: pid })
    .first()
